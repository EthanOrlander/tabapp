...
import { PostConfirmationConfirmSignUpTriggerEvent, APIGatewayProxyEvent } from "aws-lambda";
...
/*
 * Before sign up, check if a user exists with the same phone #
...
const preSignUpLambda = new aws.lambda.CallbackFunction('cognito-pre-signup-lambda', {
    callback: async (req: APIGatewayProxyEvent) => {
...
const authEndpoint = new awsx.apigateway.API("auth", {
    routes: [
        {
            path: "/preSignUp",
            method: "GET",
            eventHandler: preSignUpLambda
        }
    ]
});

export const preSignUpUrl = pulumi.interpolate`${authEndpoint.url}/preSignUp`;
